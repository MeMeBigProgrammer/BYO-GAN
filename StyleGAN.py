...
def display_image(
    ...
    if images.dim() == 3:  # single image
        ...
        image_grid = utils.make_grid(
            images.detach().cpu()[:num_display], nrow=int(math.sqrt(num_display))
        )
...
class StyleGAN(nn.Module):
    def __init__(self, image_progression, z_size=512, image_channels=3):
        ...
        self.gen_channel_progression = {  # Im size -> num channels output
            4: 512,
            8: 512,
            16: 512,
            32: 256,
        ...
        for index, im_size in enumerate(image_progression):
            ...
            if index != 0:
                prev_size = (image_progression[index - 1], image_progression[index - 1])

            in_chan = self.gen_channel_progression[prev_size[0]]
            out_chan = self.gen_channel_progression[out_size[0]]

            self.gan_blocks.append(
                StyleGANBlock(
                    in_chan,
                    out_chan,
                    image_color_channels=image_channels,
                    im_out=out_size,
                    im_in=prev_size,
                )
            )
...
class Critic(nn.Module):
    def __init__(self, image_progression, image_channels=3):
        ...
        self.critic_channel_progression = {  # Im size -> num channels output
            4: 512,
            8: 512,
            16: 512,
            32: 256,
        ...
        for index, im_size in enumerate(self.image_progression[::-1]):
            ...
            in_chan = critic_channel_progression.get(in_size[0], 512)
            out_chan = critic_channel_progression.get(in_size[0] / 2, 512)

            self.critic_blocks.add_module(
                f"{in_size} intake Critic Block",
                CriticBlock(
                    in_size,
            ...
            )  # TODO have dynamic critic weights
...
image_progression = [4, 8, 16, 32, 64, 128, 256, 512]
gen_channel_progression = {  # Im size -> num channels output
    4: 512,
    8: 512,
    16: 512,
    32: 256,
...
critic_channel_progression = {  # Im size -> num channels output
    4: 512,
    8: 512,
    16: 512,
    32: 256,
...
glacier_images = datasets.ImageFolder(path_root + "/glaciers", transformation)
building_images = datasets.ImageFolder(path_root + "/buildings", transformation)
forest_images = datasets.ImageFolder(path_root + "/forest", transformation)
...
def train(saved_model=None):
    ...
    gen_opt = torch.optim.Adam(
        [
            ...
            {"params": gen.gan_blocks.parameters()},
        ],
        lr=learning_rate,
        betas=(beta_1, beta_2),
    ...
    if saved_model is not None:
        ...
        critic_image_count = checkpoint["critic_image_count"]
        ...
        for x, _ in pbar:
            ...
            for i in range(critic_repeats):
                ...
                fake_noise = get_truncated_noise(
                    current_batch_size, noise_size, 0.75
                ).to(device)
                ...
                x = torch.nn.functional.interpolate(
                    x,
                    size=(fake_images.shape[2], fake_images.shape[3]),
                    mode="bilinear",
            ...
            gen_opt.zero_grad()
            noise = get_truncated_noise(current_batch_size, noise_size, 0.75).to(device)
            fake_images = gen(noise, critic_image_count, im_milestone)

            critic_fake_pred = critic(fake_images, critic_image_count, im_milestone)
...
def test():
    gen = StyleGAN(image_progression, z_size=noise_size).to(device)
    critic = Critic(image_progression).to(device)
    fake_noise = get_truncated_noise(12, 512, 0.75).to(device)
    for i in range(100):
...
if __name__ == "__main__":
    ...
    train(saved_model="./checkpoints/chk-11000.pth")
